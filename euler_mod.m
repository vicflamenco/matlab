function W = euler_mod(fty,a,b,h,alfa,solucion,e)
    syms t y;
    n=round((b-a)/h);
    x(1)=a;
    H=h;
    ...
    fprintf('\n\tW0 = %9.15f\n\n\t',alfa);
    fprintf('Wi+1 = ');
    Funcion = Wi + h/2*subs(fty,[t y],[ti Wi]) + h/2*subs(fty,[t y],[ti1 Wi+h*subs(fty,[t y],[ti Wi])]);
    pretty(Funcion);
    h=H;
    ...
    for i=1:n+1
       if i==1
            ...
            W(i)= W(i-1) +(h/2)*e1 + (h/2) * subs(fty,[t y],[x(i) W(i-1)+h*e1]) ;
       end
